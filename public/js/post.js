giveFeedback = (elem, post_id, is_like, is_comment) => {
    const url = '/post/' + post_id;
    sendAjaxRequest('put', url, { is_like }, giveFeedbackHandler(elem, post_id, is_like, is_comment));
}

removeFeedback = (elem, post_id, is_like, is_comment) => {
    const url = '/post/' + post_id;
...
const giveFeedbackHandler = (elem, post_id, is_like, is_comment) => function() {
    ...
    const likeSelector = is_comment ? `#likes_${post_id}` : '#articleLikes';
    const dislikeSelector = is_comment ? `#dislikes_${post_id}` : '#articleDislikes';
    ...
    oppositeFeedback.onclick = () => giveFeedback(oppositeFeedback, post_id, !is_like, is_comment);

    if (previousError) previousError.remove();
}

const removeFeedbackHandler = (elem, post_id, is_like, is_comment) => function() {
    if (this.status == 403) {
        window.location = '/login';
        return;
    }
    ...
    elem.onclick = () => giveFeedback(elem, post_id, is_like, is_comment);
